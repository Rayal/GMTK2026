...
var touching_settlement_tiles: Array = []
...
func spec() -> void:
	life_time_sec *= randf_range(0.5, 1.5)
	base_speed *= randf_range(0.5, 1.5)
	forest_speed_modifier = randf_range(0.4, 0.6)
	mountain_speed_modifier = randf_range(0.2, 0.4)
...
func _on_area_2d_body_shape_entered(body_rid: RID, body: Node2D, body_shape_index: int, local_shape_index: int) -> void:
	if body is TileMapLayer:
		...
		if col_layer == 1:
			...
			if touching_settlement_tiles.is_empty():
				settlement_entered.emit()
			touching_settlement_tiles.append(body_rid)
		elif col_layer == 8:
...
func _on_area_2d_body_shape_exited(body_rid: RID, body: Node2D, body_shape_index: int, local_shape_index: int) -> void:
	if body is TileMapLayer:
		...
		if col_layer == 1:
			...
			touching_settlement_tiles.erase(body_rid)
		elif col_layer == 8:
...
func _on_upgrade_choice(choice: int) -> void:
	if choice == 0:
		spec()
	elif choice == 1:
		forest_zoom *= 0.5
		forest_speed_modifier *= 1.5
	elif choice == 2:
		mountain_zoom *= 0.5
